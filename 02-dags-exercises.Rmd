...
ggdag_paths(coffee_cancer_dag) + theme_void()
ggdag_adjustment_set(coffee_cancer_dag)
```

According to ggdag_adjustment_set procedure, we can see that we need to control for "addictive behavior" and "smoking" before we can see whether or not there is actually a causal relationship between coffee and cancer.
...
lm(cancer ~ coffee + smoking + addictive, data=smoking_data)
